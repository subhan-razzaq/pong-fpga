`timescale 1 ns / 1 ps
module pong_top(input wire clk, input wire [15:0] sw, output wire Hsync, Vsync, dp, output wire [3:0] vgaRed, vgaGreen, vgaBlue, an, output wire [6:0] seg);
    wire clk_25;
    wire clk_1k;
    ...
    seven_seg_mux inst_seven_seg(.clk(clk), .i_ce(clk_1k), .i_digits(sw), .i_blank(blank), .o_seg(seg), .o_an(an)); // switches used as digits just for testing purposes now.
endmodule